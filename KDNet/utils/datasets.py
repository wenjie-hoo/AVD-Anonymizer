...
def img2label_paths(img_paths, file_path):
    # Define label paths as a function of image paths
    # img_paths: /data/zxy/datasets/CCPD2019/sub dataset name/label_name.jpg
    # file_path: /data/zxy/datasets/CCPD2019/splits/train.txt
    # label: /data/zxy/datasets/CCPD2019/splits/labels/train/label_name.txt
    # print('img_paths[0]:', img_paths[0])  # ccpd_base/label_name.jpg
    # sa, sb = os.sep + 'images' + os.sep, os.sep + 'labels' + os.sep  # /images/, /labels/ substrings
    sa = img_paths[0].split("/", -1)[-2]  # ccpd_base
    # label_name = img_paths[0].split("/", 1)[1]  # label_name.jpg
    # print('sa:', sa)
    path = file_path.split("/", -1)[5] + "/labels/"  # splits/labels/
    sub_name = file_path.split("/", -1)[6].split(".", 1)[0]  # sub dataset name
    # print('path:', path)
    # print('sub_name:', sub_name)
    path = path + sub_name
    print("path:", path)
    # print('path:', path)  # splits/labels/sub dataset name/
    # return ['txt'.join(x.replace(sa, sb, 1).rsplit(x.split('.')[-1], 1)) for x in img_paths]
    return [
        "txt".join(x.replace(sa, path, 1).rsplit(x.split(".")[-1], 1))
        for x in img_paths
    ]


# def img2label_paths1(img_paths):
#     label_paths = []
#     for x in img_paths:
#         sa = x.split("/", -1)[-2]  # ccpd_base
#         path = 'splits/labels/' + sa  # splits/labels/sub dataset name/
#         label_paths += ['txt'.join(x.replace(sa, path, 1).rsplit(x.split('.')[-1], 1))]
#     # print('label:', label_paths[1])  # /data/zxy/datasets/CLPD/splits/labels/CLPD_1200/0.txt
#     # print('label:', label_paths[99])  # /data/zxy/datasets/CLPD/splits/labels/CLPD_1200/0.txt
#     return label_paths
...
def img2label_paths1(img_paths):
    label_paths = []
    for x in img_paths:
        # Instead of "splits/labels/" + sa:
        # just replace "images" with "labels"
        x_new = x.replace("/images/", "/labels/").rsplit(".", 1)[0] + ".txt"
        label_paths.append(x_new)
    return label_paths
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(
            ...
            for p in path if isinstance(path, list) else [path]:
                ...
                if p.is_dir():  # dir
                    ...
                    with open(p, "r") as t:
                        ...
                        parent = str(p.parent).split("/splits", -1)[0] + os.sep
                        # print('parent:', parent)  # /data/zxy/datasets/CCPD2019/
                        f += [
                            os.path.join(parent, x) for x in t
        ...
        # Check cache
        self.label_files = img2label_paths1(self.img_files)  # labels  # AOLP
        # image_path = self.path + self.img_files
        # self.label_files = img2label_paths(self.img_files, self.path)  # labels  # CCPD、CLPD
        ...
        self.label_files = img2label_paths1(cache.keys())  # update
        if single_cls:
