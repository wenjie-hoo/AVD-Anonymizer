...
def make_gt_boxes(gt_boxes, max_num_box, batch, img_size):
    ...
    for i in range(batch):
        ...
        if num_boxes < max_num_box:
            gt_boxes_padding = torch.zeros(
                [max_num_box, gt_boxes.size(1)], dtype=torch.float
            )
    ...
    new_gt_boxes_aim[:, :, 2] = (
        new_gt_boxes[:, :, 2] - 0.5 * new_gt_boxes[:, :, 4]
    ) * img_size[1]
    new_gt_boxes_aim[:, :, 3] = (
        new_gt_boxes[:, :, 3] - 0.5 * new_gt_boxes[:, :, 5]
    ) * img_size[0]
    new_gt_boxes_aim[:, :, 4] = (
        new_gt_boxes[:, :, 2] + 0.5 * new_gt_boxes[:, :, 4]
    ) * img_size[1]
...
def getMask(batch_size, gt_boxes, img_size, feat, anchors, max_num_box, device):
    ...
    IOU_map = bbox_overlaps_batch(all_anchors, gt_boxes, img_size).view(
    ...
    for i in range(batch_size):
        max_iou, _ = torch.max(
            IOU_map[i].view(height * width * A, gt_boxes.shape[1]), dim=0
        )
...
def compute_mask_loss(
    mask_batch, student_feature, teacher_feature, imitation_loss_weight
):
    student_bs = student_feature.size(0)
    teacher_bs = teacher_feature.size(0)
    mask_bs = len(mask_batch)
    min_bs = min(student_bs, teacher_bs)
    if mask_bs > min_bs:
        mask_batch = mask_batch[:min_bs]
    ...
    student_feature = student_feature[:min_bs]
    teacher_feature = teacher_feature[:min_bs]

    norms = (
        mask_batch.sum() * 2 if mask_batch.sum() > 0 else 1.0
    ...
    sup_loss = (
        torch.pow(teacher_feature - student_feature, 2) * mask_batch_no
    ).sum() / norms
    sup_loss = sup_loss * imitation_loss_weight
    return sup_loss
