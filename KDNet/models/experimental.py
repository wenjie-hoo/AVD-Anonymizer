...
def attempt_load_zxy(weights, device, map_location=None):
    # Loads an ensemble of models weights=[a,b,c] or a single model weights=[a] or weights=a
    model = Ensemble()
    for w in weights if isinstance(weights, list) else [weights]:
        attempt_download(w)
        ...
        ckpt = torch.load(w, map_location=map_location, weights_only=False)
        model.append(
            ckpt["ema" if ckpt.get("ema") else "model"].to(device).float().fuse().eval()
        )  # FP32 model
...
def attempt_loadv5(weights, device=None, inplace=True, fuse=True):
    # Loads an ensemble of models weights=[a,b,c] or a single model weights=[a] or weights=a
