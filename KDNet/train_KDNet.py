...
from utils.distill_utils import getMask, compute_mask_loss
    ...
    for epoch in range(
        ...
        for i, (
            ...
            with amp.autocast(enabled=cuda):
                ...
                # distillation model
                feature1, feature2, feature3 = pred
                # compute imitation loss
                anchors = compute_loss.anchors.cpu().numpy()  # 锚框
                mask1 = getMask(
                    batch_size,
                    targets,
                    imgs.shape[2:],
                ...
                teacher.eval()
                with torch.no_grad():
                    outs, train_out = teacher(imgs)
                    feature1_t, feature2_t, feature3_t = train_out
                    loss_d_1 = compute_mask_loss(
                        mask1, feature1, feature1_t, opt.distill_ratio
                    )
                    loss_d_2 = compute_mask_loss(
                        mask2, feature2, feature2_t, opt.distill_ratio
                    )
                    loss_d_3 = compute_mask_loss(
                        mask3, feature3, feature3_t, opt.distill_ratio
                    )
                    loss_d = loss_d_1 + loss_d_2 + loss_d_3

                loss += loss_d
                loss_items = torch.cat(
                    (loss_items, loss_d.unsqueeze(0), loss_d.unsqueeze(0))
                )
                loss_items += torch.cat(
                    (
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--cfg", type=str, default="cfg/training/KDNet.yaml", help="model.yaml path"
    )
    parser.add_argument(
        "--data", type=str, default="data/ccpd.yaml", help="data.yaml path"
    )
    parser.add_argument(
        "--hyp",
        type=str,
    ...
    parser.add_argument("--teacher-weights", type=str, default="models/yolov7.pt")
    parser.add_argument("--distill-ratio", type=float, default=0.01)
    parser.add_argument("--max_num_box", type=int, default=100)
    ###
    parser.add_argument(
    ...
    parser.add_argument("--name", default="KDNet", help="save to project/name")
    parser.add_argument(
        "--exist-ok",
        action="store_true",
